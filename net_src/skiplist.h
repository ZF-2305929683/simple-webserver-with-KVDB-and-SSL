...
public:
    ...
    static SkipList<K,V>* Get_skiplist(int maxlevel)
    {
        if (instance == nullptr) 
        {
            std::cout << "Creat Instance" << std::endl;
            instance = new SkipList(maxlevel);
        }
        else std::cout << "Instance Exist" << std::endl;
        return instance;
    }
private:
...
template<typename K, typename V>
SkipList<K, V>* SkipList<K, V>::instance = nullptr;
